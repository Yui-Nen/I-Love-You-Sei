...
const HomePage = () => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const done = localStorage.getItem('done') === 'true';
            setIsDone(done);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            ...
            setIsDone(true);  // Update state immediately
            toast({
                    ...
                    <form onSubmit={handleSubmit} className='bg-white'>
                        <div className='flex rounded-full bg-white border'>
                            {isDone ? (
                                ...
                                <>
                                    <Input
                                        className='focus-visible:ring-0 text-sm rounded-full bg-white border-none'
                                        placeholder='Anong Message mo sakin? :('
                                        value={message}
                                        onChange={(e) => setMessage(e.target.value)}
                                    />
                                    <button className='bg-[#8464b8] z-10 px-5 text-white rounded-e-full' disabled={loading}>
                                        {loading ? 'Wait' : 'Send'}
                                    </button>
                                </>
                            )}
